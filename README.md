...
- **长期记忆召回** - 长期记忆本地向量化（SQLite 存储 + 余弦相似度检索），自动召回相关记忆注入回复上下文，也可主动查询，可选配 Embedding 模型
...
- **长期记忆注入阈值** - 消息入库时召回长期记忆的相似度门槛，默认 0.45
...
配置后，短期记忆迁移到长期记忆时自动向量化入库；每条消息入库时后台召回相关长期记忆并注入回复上下文，也可通过 `recall_memory` 工具按余弦相似度主动查询。未配置时长期记忆自动停用，仅保留短期记忆。
