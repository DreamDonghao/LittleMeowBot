...
namespace insoulforge::OneBotClient {
    ...
    [[nodiscard]] drogon::Task<std::optional<uint64_t>> sendGroupMsg(uint64_t groupId, const std::string &message);
    ...
    [[nodiscard]] drogon::Task<bool> setCustomFaceDesc(const std::string &emojiId, const std::string &resId,
      const std::string &md5, const std::string &desc, std::optional<uint64_t> sessionId = std::nullopt);
