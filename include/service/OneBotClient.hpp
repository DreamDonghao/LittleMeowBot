...
namespace insoulforge::OneBotClient {
    ...
    [[nodiscard]] drogon::Task<std::optional<uint64_t> > sendGroupMsg(
        uint64_t groupId, const std::string &message, std::optional<uint64_t> sessionId = std::nullopt);

    /// @brief 发送私聊消息
    /// @note OneBot 11: send_private_msg
    /// @return message_id；失败返回 nullopt
    [[nodiscard]] drogon::Task<std::optional<uint64_t> > sendPrivateMsg(
        uint64_t userId, const std::string &message, std::optional<uint64_t> sessionId = std::nullopt);

    /// @brief 群组单人禁言
    /// @note OneBot 11: set_group_ban（duration=0 解除禁言）
    [[nodiscard]] drogon::Task<bool> setGroupBan(
        uint64_t groupId, uint64_t userId, uint64_t duration);
    ...
    [[nodiscard]] drogon::Task<bool> deleteMsg(
        uint64_t messageId, std::optional<uint64_t> sessionId = std::nullopt);
    ...
    [[nodiscard]] drogon::Task<std::optional<std::string> > getImage(
        const std::string &file, std::optional<uint64_t> sessionId = std::nullopt);
    ...
    [[nodiscard]] drogon::Task<std::optional<std::string> > downloadFile(
        const std::string &url, std::optional<uint64_t> sessionId = std::nullopt);
