...
namespace insoulforge {
    ...
    SessionConfig SessionStore::getSessionConfig(const uint64_t sessionId) const {
        ...
        Statement stmt(db.handle(), "SELECT all_mes_count, all_char_count FROM group_config WHERE group_id = ?");
    ...
    bool SessionStore::hasSessionConfig(const uint64_t sessionId) const {
        auto &db = Database::instance();
        std::shared_lock lock(db.mutex());
        Statement stmt(db.handle(), "SELECT 1 FROM group_config WHERE group_id = ?");
        stmt.bind(1, sessionId);
        return stmt.step();
    }
    ...
    bool SessionStore::isSessionEnabled(const uint64_t sessionId) const {
        auto &db = Database::instance();
        std::shared_lock lock(db.mutex());
        Statement stmt(db.handle(), "SELECT enabled FROM enabled_groups WHERE group_id = ?");
        stmt.bind(1, sessionId);
    ...
    void SessionStore::enableSession(const uint64_t sessionId) const {
        auto &db = Database::instance();
        std::unique_lock lock(db.mutex());
        Statement stmt(db.handle(), "INSERT OR REPLACE INTO enabled_groups (group_id, enabled) VALUES (?, 1)");
        stmt.bind(1, sessionId);
        stmt.exec();
    ...
    void SessionStore::disableSession(const uint64_t sessionId) const {
        auto &db = Database::instance();
        std::unique_lock lock(db.mutex());
        Statement stmt(db.handle(), "DELETE FROM enabled_groups WHERE group_id = ?");
        stmt.bind(1, sessionId);
        stmt.exec();
    ...
    std::vector<uint64_t> SessionStore::getEnabledGroups() const {
        ...
        Statement stmt(db.handle(), "SELECT group_id FROM enabled_groups WHERE enabled = 1");
        while (stmt.step()) {
    ...
    void SessionStore::toggleSessionStatus(const uint64_t sessionId) const {
        auto &db = Database::instance();
        std::unique_lock lock(db.mutex());
        Statement stmt(db.handle(), "UPDATE enabled_groups SET enabled = NOT enabled WHERE group_id = ?");
        stmt.bind(1, sessionId);
        stmt.exec();
    }
    ...
    void SessionStore::updateSessionName(const uint64_t sessionId, const std::string &name) const {
        auto &db = Database::instance();
        std::unique_lock lock(db.mutex());
        Statement stmt(db.handle(), "UPDATE enabled_groups SET group_name = ? WHERE group_id = ?");
        stmt.bind(1, name);
        stmt.bind(2, sessionId);
        stmt.exec();
    ...
    std::string SessionStore::getSessionName(const uint64_t sessionId) const {
        auto &db = Database::instance();
        std::shared_lock lock(db.mutex());
        Statement stmt(db.handle(), "SELECT group_name FROM enabled_groups WHERE group_id = ?");
        stmt.bind(1, sessionId);
