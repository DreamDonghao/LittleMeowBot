...
namespace insoulforge {
    ...
    LongTermMemoryStore &LongTermMemoryStore::instance() {
        static LongTermMemoryStore store;
        return store;
    }

    bool LongTermMemoryStore::addMemory(
      const uint64_t groupId, const std::string &content, const std::vector<float> &embedding) const {
    ...
    std::vector<LongTermMemoryEntry> LongTermMemoryStore::listMemories(
      const uint64_t sessionId, const int limit, const int offset) const {
        const auto &db = Database::instance();
        std::shared_lock lock(db.mutex());
    ...
    int64_t LongTermMemoryStore::countMemories(const uint64_t sessionId) const {
        const auto &db = Database::instance();
        std::shared_lock lock(db.mutex());
    ...
    bool LongTermMemoryStore::deleteMemory(const int64_t id) const {
        const auto &db = Database::instance();
        std::unique_lock lock(db.mutex());
