...
namespace LittleMeowBot {
    ...
    void LogWebSocketManager::updateSubscription(const drogon::WebSocketConnectionPtr& conn, LogSubscription subscription) {
        std::lock_guard lock(m_mutex);
    ...
    bool LogWebSocketManager::matches(const LogSubscription& subscription, const Json::Value& log) {
        ...
        if (!subscription.keyword.empty() && log.get("message", "").asString().find(subscription.keyword) == std::string::npos) {
            return false;
        }
        return true;
    }
