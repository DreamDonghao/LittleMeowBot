...
namespace insoulforge {
    void AgentToolManager::registerAllTools() {
        ...
        registry.registerTool({.name = "reply",
        ...
        quoteReplyParams["properties"]["message_id"]["description"] =
          "要引用的消息ID（从聊天记录JSON的message_id字段获取）";
            ...
            .handler = [](const Json::Value &args) -> drogon::Task<std::string> {
                ...
                if (emoji.get("is_mark_face", false).asBool() && !emoji["emoji_id"].asString().empty() &&
                    ...
                    co_return fmt::format("[CQ:mface,summary={},emoji_id={},emoji_package_id={},key={}]",
                      emoji["summary"].asString(), emoji["emoji_id"].asString(), emoji["emoji_package_id"].asString(),
                      emoji["key"].asString());
                }
        ...
        saveParams["properties"]["name"]["description"] = "给表情起的简短名字（根据图片内容），如: 摸头、猫猫惊讶";
        ...
        atParams["properties"]["qq"]["description"] =
          "要@的QQ号（从聊天记录JSON的sender.qq字段获取）。使用 'all' @全体成员";
        ...
        banParams["properties"]["duration"]["description"] =
          "禁言时长（秒）。轻度60-300秒，中度600-1800秒，重度3600秒以上。0解除禁言";
        ...
        recallParams["properties"]["message_id"]["description"] =
          "要撤回的消息ID（从聊天记录JSON的message_id或reply_to字段获取）";
    ...
    void AgentToolManager::registerCustomTools() {
        ...
        for (const auto &tool: tools) {
            ...
            if (!tool.parameters.empty()) {
                ...
                std::string errors;
                reader->parse(
                  tool.parameters.c_str(), tool.parameters.c_str() + tool.parameters.size(), &params, &errors);
